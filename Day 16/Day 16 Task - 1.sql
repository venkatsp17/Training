...
select type,avg(price) Price from titles group by type
...
select pub_id, count(*) "Number of Books" from titles where (price between 15 and 25) and (title like '%It%') group by pub_id having (count(*) > 2) order by count(pub_id) asc
