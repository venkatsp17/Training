...
{
    public class AppointmentBLGetAppointmentsByStatusTest
    {
        ...
        [Test]
        public void GetAppointmentByStatusSuccessTest()
        {
            //Arrange
            ...
            //Action
            var result = appointmentServices.GetAppointmentsByStatus("Initiated");
            //Assert
            Assert.That(result[0].AppointmentId, Is.EqualTo(1));
        }

        [Test]
        public void GetAppointmentByStatusAppointmentNotFoundExceptionTest()
        {
            //Arrange
            Doctor doctor = new Doctor() { Age = 40, Experience = 10, LicenseNumber = "A123", Name = "JOHN", PhoneNumber = "1112223334", Qualification = "MD", Specialization = "ENT" };
            Patient patient = new Patient() { Name = "JOHN", PhoneNumber = "1112223334", Address = "TVL", DateOfBirth = Convert.ToDateTime("2002-09-01"), Gender = "Male" };
            ...
            appointmentRepository.Add(appointment);
            appointmentServices = new AppointmentBL(appointmentRepository);
            var exception = Assert.Throws<AppointmentNotFoundException>(() => appointmentServices.GetAppointmentsByStatus("Cancelled"));
            //Assert
            Assert.That(exception.Message, Is.EqualTo("Appointment Not Found!"));
        }

        [Test]
        public void GetAppointmentByStatusNotDataAvailabeleExceptionTest()
        {
            //Arrange
            appointmentServices = new AppointmentBL(appointmentRepository);
            //Action
            var exception = Assert.Throws<NoDataAvailableException>(() => appointmentServices.GetAppointmentsByStatus("Initiated"));
            //Assert
