...
{
    public class AppointmentBLGetAppointmentsForDoctorTest
    {
        ...
        [Test]
        public void GetAppointmentsForDoctorSuccessTest()
        {
            //Arrange
            ...
            //Action
            var result = appointmentServices.GetAppointmentsForDoctor(1);
            //Assert
            Assert.That(result[0].AppointmentId, Is.EqualTo(1));
        }

        [Test]
        public void GetAppointmentsForDoctorAppointmentNotFoundExceptionTest()
        {
            //Arrange
            ...
            appointmentRepository.Add(appointment);
            appointmentServices = new AppointmentBL(appointmentRepository);
            var exception = Assert.Throws<AppointmentNotFoundException>(() => appointmentServices.GetAppointmentsForDoctor(2));
            //Assert
            Assert.That(exception.Message, Is.EqualTo("Appointment Not Found!"));
        }

        [Test]
        public void GetAppointmentsForDoctorNotDataAvailabeleExceptionTest()
        {
            //Arrange
            appointmentServices = new AppointmentBL(appointmentRepository);
            //Action
            var exception = Assert.Throws<NoDataAvailableException>(() => appointmentServices.GetAppointmentsForDoctor(1));
            //Assert
